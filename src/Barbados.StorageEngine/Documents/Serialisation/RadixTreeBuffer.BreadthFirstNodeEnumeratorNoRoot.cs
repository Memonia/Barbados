...
namespace Barbados.StorageEngine.Documents.Serialisation
{
	...
	{
		private ref struct BreadthFirstNodeEnumeratorNoRoot
		{
			...
			public BreadthFirstNodeEnumeratorNoRoot(ReadOnlySpan<byte> buffer)
			{
				if (_isEmpty(buffer))
				{
			...
			public BreadthFirstNodeEnumeratorNoRoot(ReadOnlySpan<byte> buffer, int rootOffset)
			{
				if (_isEmpty(buffer))
				{
					_currentGroupEnumerator = new(buffer, -1);
