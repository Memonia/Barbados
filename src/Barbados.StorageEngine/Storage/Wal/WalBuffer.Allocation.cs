...
namespace Barbados.StorageEngine.Storage.Wal
{
	internal partial class WalBuffer
	{
		...
		public static ulong AllocateRootAndGetMagic(IStorageWrapper storage)
		{
			...
			var cpage = new BTreePage(cpageHandle);
			var ipage = new BTreePage(ipageHandle);
		...
		{
			var index = handle.Handle / AllocationPage.PageCountPerBitmap;
			var allocHandle = new PageHandle(index * AllocationPage.PageCountPerBitmap);
		...
		public PageHandle Allocate(Snapshot snapshot)
		{
			bool _tryGetFreePageHandle(out PageHandle handle)
			{
				...
				bitmapIndex += 1;
				var bitmapHandle = new PageHandle(AllocationPage.PageCountPerBitmap);
				while (bitmapHandle.Handle < root.LastAllocationPageHandle.Handle)
				{
					...
					bitmapIndex += 1;
					bitmapHandle = new PageHandle(bitmapHandle.Handle + AllocationPage.PageCountPerBitmap);
				}
