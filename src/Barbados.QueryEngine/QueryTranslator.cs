    ...
    internal static class QueryTranslator
    {
        ...
        )
        {
            if (expression is ComparisonExpression ce)
            {
                ...
                FieldExpression fe;
                IQueryExpression coe;
                if (ce.Left is FieldExpression)
                {
                    fe = (FieldExpression)ce.Left;
                    coe = ce.Right;
                }

                else
                if (ce.Right is FieldExpression) 
                { 
                    fe = (FieldExpression)ce.Right;
                    coe = ce.Left;
                }
                ...
                if (coe.GetType().GetGenericTypeDefinition() != typeof(ConstantExpression<>))
                {
                    evaluator = default!;
                    return false;
                }
                ...
                object value = coe switch
                {
                    ...
                    _ => throw new ArgumentException($"Constant expression of type {coe.GetType().GetGenericArguments()[0]} is not supported")
                };
                ...
                evaluator = new IndexSeekEvaluator(collection, fe.Name, fob.Build());
                return true;
            }
