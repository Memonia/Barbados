...
namespace Barbados.Documents.RadixTree
{
	internal partial class RadixTreeBuffer
	{
		...
		{
			public ref struct BreadthFirstNodesNoRoot
			{
				...
				private BreadthFirstSingleLevel _currentGroupEnumerator;
				...
				public BreadthFirstNodesNoRoot(ReadOnlySpan<byte> buffer)
				{
					if (IsEmpty(buffer))
					{
				...
				public BreadthFirstNodesNoRoot(ReadOnlySpan<byte> buffer, int rootOffset)
				{
					if (IsEmpty(buffer))
					{
						_currentGroupEnumerator = new(buffer, -1);
