...
namespace Barbados.Documents.RadixTree
{
	internal partial class RadixTreeBuffer
	{
		...
		public static RadixTreeBuffer ExtractWithPrefix(ReadOnlySpan<byte> buffer, RadixTreePrefixSpan prefix)
		{
			...
			var rootOffset = _getNodeOffset(buffer, prefix.AsBytes());
			if (rootOffset < 0)
			{
